...
void main() {
  ...
  vec4 waterSampleLeft = texture2D(water, textureCoordinates + texturePixelSize * vec2(-1, 0));
  vec4 waterSampleRight = texture2D(water, textureCoordinates + texturePixelSize * vec2(1, 0));
  vec4 waterSampleDown = texture2D(water, textureCoordinates + texturePixelSize * vec2(0, -1));
  vec4 waterSampleUp = texture2D(water, textureCoordinates + texturePixelSize * vec2(0, 1));
...
/*  vec4 bg1 = vec4(0.86, 0.86, 0.86, 1.0);
  ...
  //dryContribution.rgba *= 2.0;
  //wetContribution.rgba *= 2.0;
